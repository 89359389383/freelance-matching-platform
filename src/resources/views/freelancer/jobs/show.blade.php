        ...
        <div class="content-area">
            ...
            <section class="section" aria-label="応募">
                <div class="btn-row horizontal flex flex-col md:flex-row gap-3 md:gap-4">
                    @if($alreadyApplied)
                        @if($thread)
                            <a href="{{ route('freelancer.threads.show', $thread->id) }}" class="btn btn-primary w-full md:flex-1">応募済み（チャットを開く）</a>
                        @else
                            <button class="btn btn-primary w-full md:flex-1" disabled>応募済み</button>
                        @endif
                    @else
                        <a href="{{ route('freelancer.jobs.apply.create', $job->id) }}" class="btn btn-primary w-full md:flex-1">応募する</a>
                    @endif
                    <a href="{{ route('freelancer.jobs.index') }}" class="btn btn-secondary w-full md:flex-1">一覧に戻る</a>
                </div>
            </section>
        </div>
    </main>
